...
| **The drawn notch** | On a display without a notch of its own, the anchor the island grows from is our rectangle rather than a camera housing &mdash; so it carries the plan (`Max 20x`) between the two chips, at the notch's exact width. Hidden, never clipped, if a label will not fit; hidden while the panel is open, where the header says it already. | Notchless displays |
